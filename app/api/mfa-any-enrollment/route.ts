...
export async function POST(request: NextRequest) {
  try {
    ...
    if (step === 'initiate') {
      ...
      const ticketResponse = await fetch(`https://${domain}/api/v2/guardian/enrollments/ticket`, {
        method: 'POST',
        headers: {
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const deleteUrl = `https://${domain}/api/v2/users/${session.user.sub}/enrollments/${enrollmentId}`;
