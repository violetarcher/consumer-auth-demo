/**
 * ConsumerAuth - Auth0 Customized Consent Screen
 * Dramatic split-screen design for user consent
 */

document.addEventListener('DOMContentLoaded', function() {
  console.log('DOM loaded, initializing ConsumerAuth customized-consent screen...');
  ...
  async function initializeCustomizedConsent() {
    console.log('Starting customized-consent initialization...');

    const auth0SDK = await waitForAuth0SDK();

    if (!auth0SDK || !auth0SDK.CustomizedConsent) {
      console.error('❌ CustomizedConsent not available');
      return;
    }

    // Initialize ACUL CustomizedConsent manager
    const consentManager = new auth0SDK.CustomizedConsent();
    console.log('✅ CustomizedConsent manager initialized:', consentManager);
  ...
  initializeCustomizedConsent().catch(error => {
